...
class TestStockStatusStability(unittest.TestCase):
    ...
    def test_quotation_stock_status_has_single_source_and_race_guard(self):
        ...
        self.assertIn("before_workflow_action(frm)", script)
        self.assertIn('frm.selected_workflow_action !== "Reject"', script)
        self.assertIn("frm.doc.order_lost_reason", script)
        self.assertIn("frappe.dom.unfreeze()", script)
        self.assertIn("frappe.msgprint({", script)
        self.assertIn("return Promise.reject", script)
        self.assertNotIn("frappe.throw", script)
